...
class MiniLMTrainer(Trainer):
    ...
    def on_epoch_begin(self, args, state, control, **kwargs):
        ...
        if current_epoch < 1:
            current_epoch = 1
        print(f"Starting epoch {current_epoch} training... (state.epoch={state.epoch})")
        return control
    ...
    def on_epoch_end(self, args, state, control, **kwargs):
        ...
        if current_epoch > self._epoch:
            ...
            print(f"Saving epoch {current_epoch} model...")
            self.save_model(epoch_output_dir)
            print(f"Epoch {current_epoch} model saved to {epoch_output_dir}")
            ...
            if self.current_epoch_best_state is not None:
                ...
                try:
                    ...
                    print(f"Best model of epoch {current_epoch} (loss: {self.current_epoch_best_loss:.4f}) saved to {epoch_best_dir}")
            ...
            print(f"State epoch: {state.epoch}, Current epoch: {current_epoch}, Previous epoch: {self._epoch}")
            ...
            try:
                ...
                logger.error(f"Error generating embeddings for epoch {current_epoch}: {str(e)}")
            ...
            print(f"Skipping save for epoch {current_epoch} (already saved, state.epoch={state.epoch}, self._epoch={self._epoch})")
    ...
    def _generate_embeddings_for_checkpoint(self, checkpoint_path: str):
        logger.info(f"Generating embeddings for checkpoint {checkpoint_path}...")
        
        model_hf_identifier = self.model.model.name_or_path
        ...
        drug_embeddings_cls = torch.cat(drug_embeddings_cls, dim=0)
        drug_embeddings_pooler = torch.cat(drug_embeddings_pooler, dim=0)
        target_embeddings_cls = torch.cat(target_embeddings_cls, dim=0)
        target_embeddings_pooler = torch.cat(target_embeddings_pooler, dim=0)
        
        for embed_type, drug_emb, target_emb in [
            ('cls', drug_embeddings_cls, target_embeddings_cls),
            ('pooler', drug_embeddings_pooler, target_embeddings_pooler)
        ]:
            suffix = f"-best-{embed_type}" if is_best else f"-{embed_type}"
            output_filename = f"{base_model_name_for_filename}-epoch-{epoch}{suffix}-embeddings.pt"
