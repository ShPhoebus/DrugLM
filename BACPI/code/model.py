...
class BACPI(nn.Module):
    ...
    def bidirectional_attention_prediction(self, atoms_vector, atoms_mask, fps, amino_vector, amino_mask, params, device,
        ...
        if params.our_data:
            protein_embeddings = []
            drug_embeddings = []
            for id in raw_protein_ids:
                protein_embeddings.append(target_dict[str(id.item())])
            for id in raw_drug_ids:
                drug_embeddings.append(drug_dict[str(id.item())])
            protein_emb = np.array(protein_embeddings, dtype=np.float64)
            drug_emb = np.array(drug_embeddings, dtype=np.float64)
            # print(f'{cf_final.shape} {pf_final.shape} {b}')
            pf_final = torch.cat([pf_final, torch.tensor(protein_emb, dtype=torch.float32).to(device)], dim=1)
            cf_final = torch.cat([cf_final, torch.tensor(drug_emb, dtype=torch.float32).to(device)], dim=1)

        cf_pf = F.leaky_relu(torch.matmul(cf_final.view(b, -1, 1), pf_final.view(b, 1, -1)).view(b, -1), 0.1)
        return self.output(cf_pf)

    def forward(self, atoms, atoms_mask, adjacency, amino, amino_mask, fps, params, device,
                raw_protein_ids=None, raw_drug_ids=None, target_dict=None, drug_dict=None):
        ...
        prediction = self.bidirectional_attention_prediction(atoms_vector, atoms_mask, super_feature, amino_vector, amino_mask, params, device,
                                                             raw_protein_ids, raw_drug_ids, target_dict, drug_dict)
        return prediction
