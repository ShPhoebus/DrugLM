...
def extract_input_data(input_path, output_path, radius, ngram, dataset, name):
    data = pd.read_csv(input_path + '.txt', header=None)
    compounds, adjacencies, fps, proteins, interactions = [], [], [], [], []

    # if our dataset find the mapping
    if dataset == 'our_data':
        protein_df = pd.read_csv(f'../data/interaction/{dataset}/proteins_{name}.csv')
        drug_df = pd.read_csv(f'../data/interaction/{dataset}/drugs_{name}.csv')
        protein_dict = dict(zip(protein_df['raw_sequence'], protein_df['id']))
        drug_dict = dict(zip(drug_df['SMILES'], drug_df['id']))
        raw_protein_id = []
        raw_drug_id = []
    ...
    for index in range(len(data)):
        ...
        if dataset =='our_data':
            raw_protein_id.append(protein_dict[sequence])
            raw_drug_id.append(drug_dict[smiles])
    ...
    if dataset == "our_data":
        np.save(os.path.join(output_path, 'raw_protein_ids'), raw_protein_id)
        np.save(os.path.join(output_path, 'raw_drug_ids'), raw_drug_id)
...
def training_data_process(task, dataset):
    ...
    for name in ['train', 'test']:
        ...
        extract_input_data(input_path, output_path, radius, ngram, dataset, name)
