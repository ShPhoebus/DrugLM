...
def batch2tensor(batch_data, device, our_data):
    ...
    if our_data:
        raw_protein_ids = batch_data[5]
        raw_drug_ids = batch_data[6]
    else:
        raw_protein_ids = None
        raw_drug_ids = None
...
def load_data(datadir, target_type, our_data):
    if target_type:
    ...
    if our_data:
        raw_protein_ids = np.load(dir_input + 'raw_protein_ids.npy', allow_pickle=True)
        raw_drug_ids = np.load(dir_input + 'raw_drug_ids.npy', allow_pickle=True)
        data_pack = [compounds, adjacencies, fingerprint, proteins, interactions, raw_protein_ids, raw_drug_ids]
    else:
